...
with mlflow.start_run():
    ...
    # -------------------- SAVE MODEL AS PKL --------------------
    with open("linear_model-2.pkl", "wb") as f:
        pickle.dump(model, f)

    # -------------------- LOG ARTIFACT --------------------
    mlflow.log_artifact("linear_model-2.pkl")#artifact mean the file you want to log and save in mlflow

    # -------------------- LOG MODEL TO MLflow --------------------
    mlflow.sklearn.log_model(
        sk_model=model,
        artifact_path="model",
        registered_model_name="LinearRegression-MOdel-2"
    )
